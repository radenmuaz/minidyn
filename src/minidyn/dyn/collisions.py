...
def separating_axis(q1, q2, shape1 , shape2):
    ...
    def did_overlap(v1, v2, n1, n2, f1, f2, naxes, np1_mins, np1_maxs, np2_mins, np2_maxs,
        ...
        def n_smaller(v1, v2, n1, n2, f1, f2, i, n_overlap, n_diff):
            def closest_point(p, ve, eps=1e-9):
            ...
            # n_in = n2[i_in]
            v_in = v2[f2[i_in]]
            p0 = closest_point(v_ref[0], v_in)
            p_ref = closest_point(p0, v_ref)
            p_in = closest_point(p_ref, v_in)
    ...
    (collide_flags, mtv, n_ref, p_ref, p_in) = res[0],res[1],res[2],res[3],res[4]
    did_collide = collide_flags[0]
    face2face = collide_flags[1]
    edge2edge = collide_flags[2]
    # (collide_flags, mtvs, nrefs, p_refs, p_ins) = jnp.split(res, 5)
    # return  (did_collide, face2face, edge2edge, mtv, n_ref, p_ref, p_in)
    return  {'did_collide':did_collide, 
            'face2face':face2face,'edge2edge':edge2edge,
            'mtv':mtv, 'n_ref': n_ref, 'p_ref': p_ref, 'p_in':p_in}

        
    
if __name__ == '__main__':
    q1 = jnp.array([1, 0, 0, 0., 0, 0, 0]) 
    q2 = jnp.array([1, 0, 0, 0., 0.1, 0.1, 0.1]) # collide
    ...
    s1 = mdn.dyn.body.Shape.from_trimesh(trimesh.creation.box((1., 1, 1.)))
    s2 = mdn.dyn.body.Shape.from_trimesh(trimesh.creation.box((1., 1., 1.)))

    res = separating_axis(jnp.stack((q1,q2)), (0,1), (s1,s2))
    print(res)
